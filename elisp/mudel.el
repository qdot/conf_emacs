...
(defface mudel-prompt-face
  '((t (:bold t :foreground "Black" :background "lightBlue2")))
  "MUDEL face for the prompt."
  :group 'mudel)

(defvar mudel-input-marker nil
  "The marker where input should be inserted.")
(make-variable-buffer-local 'mudel-input-marker)
...
(defcustom mudel-preoutput-filter-functions nil
  "Functions being run before each complete line of output from
the server. Each function is passed the line from the server as
an argument, and point is also in the displayed line from the
server.
...
(defcustom mudel-prompt "MUDEL>"
  "Prompt used by MUDEL.  Trailing whitespace is not required."
  :group 'mudel
...
(defun mudel-command-sender (proc str-with-prompt)
...
bound to a function, send STR unmodified to the server."
  (let* ((str (replace-regexp-in-string comint-prompt-regexp "" str-with-prompt)))
    (if (zerop (length str))
...
`comint-output-filter-functions'."
  (when (string-match "\n" string)
    ...
    ;; We consider this the last thing to be done on the output, so we
    ;; output our prompt here.
    ;; (mudel-display-prompt)
    ))
...
(defun mudel-display-prompt (&optional buffer pos prompt face)
...
`erc-prompt-face' will be used."
  (let* ((prompt (or prompt mudel-prompt))
	 (l (length prompt))
	 (ob (current-buffer)))
	...
	(put-text-property 0 (1- (length prompt))
                           'face (or face 'mudel-prompt-face)
                           prompt)
	(insert prompt))
      ;; Set the input marker
      (set-marker mudel-input-marker (point)))
