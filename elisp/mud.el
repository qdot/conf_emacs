...
;;; Cleaned up some by qDot, June 2010
;;;
...
;;; Changes by qDot
;;; - Took out pre emacs 19 compatibility. Upgrade or die.
;;; - Removed line wrapper, just use visual-line-mode now (yay emacs 23)
;;; - Added todochiku binds for pages and whispers
...
(defvar mud-echo-buffer-list nil "List of buffers that mud.el is talking to.")

(defvar mudbard-input-buffers nil "List of input buffers")
(defvar mudbard-output-buffers nil "List of output buffers")
...
(defvar mud-default 'furry "*default mud to log in to.")
...
;;; Auto-Login support
(defvar mud-default-user "NOTMYNAME" "*default username to connect with.")
(defvar mud-default-pass "NOTMYPASS" "*default password for mud-default-user.")
...
(defvar mud-default-idle-chat nil
...
(defvar mud-default-idle-time "1000 min" "*Default idle timeout")
...
(defconst mud-default-regexps-list nil
...
(progn
  ...
  (set-face-foreground 'mud-default-page-face "green")
  ...
  (set-face-foreground 'mud-default-whisper-face "yellow")

  (defvar mud-default-scream-face nil "*Face to highlight screams with.")
  (copy-face 'default 'mud-default-scream-face)
  (set-face-foreground 'mud-default-scream-face "medium slate blue")
  )
...
(defconst mud-whisper-regexp
  "^\\(You whisper,?\\|\\S-+ whispers,\\|##w>\\) .*\\([\r\n]    .*\\)*"
...
(defconst mud-scream-regexp
  "^\\(##s>\\) .*\\([\r\n]    .*\\)*"
  "Regular expression that matches fuzzball scream channels.")
...
(add-to-list 'mud-default-regexps-list
             (vector mud-scream-regexp 'mud-highlight-scream))
...
;;  (defun mud-bing (f &rest s)
;;    (save-window-excursion
;;      (find-file "~/tmp/mudbing")
;;      (message "Binging!")
;;      (goto-char (point-max))
;;      (newline)
;;      (insert-format f (mapcar (function 2str) s))
;;      )
;;  );mud-bing
;;  
;;;
...
(defun mud(&optional mud-name)
  ...
  (let (mud-bin mud-bout mud-details mud-host mud-port mud-process
    ...
    (setq mudbard-input-buffers (cons mud-bin mudbard-input-buffers))
    (setq mudbard-output-buffers (cons mud-bout mudbard-output-buffers))
    (setq mud-echo-buffer-list (cons mud-bin (cons mud-bout mud-echo-buffer-list)))
    ...
    ;(mud-bing "before mud-windows-setup")
    ...
    (if mud-output-map
      ...
      (define-key mud-output-map [f12] 'mud-clear-buffers)
    ...
    (save-excursion (set-buffer mud-bout)
                    ...
                    ;(mud-bing "use-local-map mud-output-map: %s" (current-local-map))
    ...
    (if mud-input-map
      ...
      ;(mud-bing "built mud-input-map")
)
      (define-key mud-input-map [f12] 'mud-clear-buffers)
    ...
    (progn
      ...
      (mud-init-feature mud-new 'mud-scream-face 'mud-default-scream-face)
      (mud-init-feature mud-new 'mud-whisper-face 'mud-default-whisper-face)      
      )
    ...
    ;(mud-bing "use-local-map mud-input-map -- yurf -- %s" mud-input-map)
    (use-local-map (copy-keymap mud-input-map))
    ;(mud-bing "used-local-map! %s" (current-local-map))
    (when nil
  ...
  ;(mud-bing "end of mud")
;; I dunno why, but v23 seems to require this: 

(mudbard-mapfix)
)


(defun mudbard-mapfix() (interactive)
;  (use-local-map (copy-keymap mud-input-map))
  (let ((cb (current-buffer)))
    (dolist (ib mudbard-input-buffers)
      (when (buffer-live-p ib)
        (set-buffer ib)
        (use-local-map (copy-keymap mud-input-map))))
    (dolist (ob mudbard-output-buffers)
      (when (buffer-live-p ob)
        (set-buffer ob)
        (use-local-map (copy-keymap mud-output-map))))
    (set-buffer cb)
  ))
...
(defun mud-windows-setup( mud-current )
  ...
  (interactive)
  ;(mud-bing "mud-windows-setup: start")  
  (let ((wout (get mud-current 'mud-wout))
        ...
        mud-wheight
c3        mud-wheight-diff)
    ;(mud-bing "mud-windows-setup: after let")  
    ...
    ;(mud-bing "mud-windows-setup: after output window findie")  
    ;; Add the buffer
    (put mud-current 'mud-wout wout)
    (set-window-buffer wout bout)
    ;(mud-bing "mud-windows-setup: after set-window-buffer wout bout")  
    ...
    (if (and (windowp win)
        ...
        (if (eq win wout)
            ...
            ()
          (delete-window win)))
    ;(mud-bing "mud-windows-setup: after big if windowp win")  
    ...
    (select-window win)
    ;(mud-bing "mud-windows-setup: select-window win ")  
    (if (buffer-live-p bin)
      ...
      (set-buffer bin)
      (use-local-map (copy-keymap mud-input-map))
      ;(mud-bing "mud-windows-setup: cabbagely use-local-map")  
      )
    ...
    (if (and start
        ...
        (progn
          ...
          (if (fboundp 'todochiku-message)
              (let ((mud-name (get mud-current 'mud-name)))
                (todochiku-message 
                 (format "Whisper From %s" mud-name)
                 (format "You have been whispered at on %s" mud-name)
                 (todochiku-icon 'irc)
                 )
                )
            )
...
(defun mud-highlight-scream( mud-current &optional match-stuff )
  "Highlight screams when found."
  (let ((start (nth 0 match-stuff))
        (end (nth 1 match-stuff))
        (mud-scream-face (get mud-current 'mud-scream-face)))
    (if (and start
             end
             (facep mud-scream-face))
        (put-text-property start end 'face mud-scream-face))))
...
(defun mud-highlight-page( mud-current &optional match-stuff )
  ...
  (let ((start (nth 0 match-stuff))
    ...
    (if (fboundp 'todochiku-message)
        (todochiku-message 
         (format "Page From %s" (prin1-to-string mud-current) )
         (format "You have been paged by %s" (prin1-to-string mud-current))
         (todochiku-icon 'irc)
         )
      )
    ))
...
(defun mud-highlight-url( mud-current &optional match-stuff)
  ...
  (let ((last 0)
    ...
    (if (facep mud-url-face)
        (let ((link-start (nth n match-stuff))
              (link-end (nth (+ n 1) match-stuff))
              ;; (mud-link-map (make-sparse-keymap))
              )
          (put-text-property link-start link-end
                             'face mud-url-face)
          ;; (define-key mud-link-map [mouse-2] '(lambda () (message (format "url: %s" url))))
          ;; (put-text-property link-start link-end 'keymap mud-link-map)
          )
      )

    (add-text-properties (nth n match-stuff) (nth (+ n 1) match-stuff)
                         '(mouse-face highlight help-echo "mouse-2: visit"))
