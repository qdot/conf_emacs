;; Retrieved from https://raw.githubusercontent.com/chenfengyuan/elisp/master/next-spec-day.el

;; How to use:
;; 1. add `(load "/path/to/next-spec-day")` to your dot emacs file.
;;
;; 2. set `NEXT-SPEC-DEADLINE` and/or `NEXT-SPEC-SCHEDULED` property of a TODO
;; task,like this:
;;
;;         * TODO test
...
;;           :END:
;;
;;     The value of NEXT-SPEC-DEADLINE will return `non-nil` if `date` is last
;;     day of month,and the value of NEXT-SPEC-SCHEDULED will return `non-nil`
;;     if `date` is the fathers' day(the third Sunday of June).
;;
...
;; argument: listp, "<2013-03-29 星期五>"' when change the TODO state,please try
...
;;
;; (2). add (add-to-list 'load-path "/path/to/org-*.*.*/lisp") to your .emacs
;; file,make sure it's before any (require 'org).If you are not sure,just insert
;; it to the first line of your dot emacs file.
;;
